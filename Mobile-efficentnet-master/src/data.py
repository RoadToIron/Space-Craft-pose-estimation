...
class Clahe(torch.nn.Module):
    ...
    def __init__(self, clip_limit: int = 30, grid_size: Tuple[int, int] = (8, 8)) -> None:
        super().__init__()
...
class GaussianNoise(torch.nn.Module):
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        if torch.rand(1).item() < self.p:
            noise = torch.normal(self.mean, self.std, img.shape)
            return img + noise
        else:
            return img
...
def prepare_speed_dataset(config):
    ...
    train_set.dataset.transform = torchvision.transforms.Compose([
        ...
        GaussianNoise(mean=0.0, std=0.02, p=0.5),
    ...
    class ClaheTransform(transforms.ToTensor):
        ...
        def __call__(self, image, label=None):
            ...
            # Convert to grayscale if necessary (assuming CLAHE works on grayscale images)
            if image.mode != 'L':
            ...
            # Apply CLAHE using kornia.enhance.equalize_clahe
            enhanced_image = kornia_enhance.equalize_clahe(image_tensor, self.clip_limit, self.grid_size)

            # Return both the enhanced image and the label (if provided)
